...
public function selectOrderById($id){
    $sql = "SELECT * FROM `int2_orders_activities` WHERE `id`=:id";
    $stmt = $this->pdo->prepare($sql);
    $stmt->bindValue(':id', $id);
    $stmt->execute();
    return $stmt->fetch(PDO::FETCH_ASSOC);
...
public function insertGegevens($data){
    ...
    if (empty($errors)) {
      $sql = "INSERT INTO `int2_orders` (`firstname`, `lastname`, `email`, `adres`, `housenumber`, `postcode`, `city`) VALUES (:firstname, :lastname, :email, :adres, :housenumber, :postcode, :city)";
      $stmt = $this->pdo->prepare($sql);
...
public function insertOrder($data){
    $sql = "INSERT INTO `int2_orders_activities` (`order_id`, `activity_id`, `quantity`) VALUES(:order_id,:activity_id,:quantity)";
    $stmt = $this->pdo->prepare($sql);
